...
void main() {
  ...
  Future<AssetRecord> cloudOnlyRecord(FakeAssetRecordStore store, {String? key = 'originals/manual_abc.jpg'}) async {
    await store.upsert(localId: 'manual:abc', contentHash: 'abc', platform: 'ios');
  ...
  test('downloads the original, points the record at it, and clears cloud-only', () async {
  ...
  test('falls through to the next target when the first does not have it', () async {
    final targetsStore = BackupTargetsStore(store: FakeSecureStore());
    await targetsStore.addS3(accessKeyId: 'a', secretAccessKey: 'b', region: 'us-east-1', bucket: 'gone', prefix: '');
    await targetsStore.addS3(accessKeyId: 'a', secretAccessKey: 'b', region: 'us-east-1', bucket: 'has-it', prefix: '');
    final recordStore = FakeAssetRecordStore();
    final record = await cloudOnlyRecord(recordStore);
    var calls = 0;
    ...
    expect(calls, 2);
    expect(restored, isNotNull);
    expect((await recordStore.getByLocalId('manual:abc'))!.localDeleted, isFalse);
  });
  ...
  test('stays cloud-only when every target fails', () async {
    ...
    expect((await recordStore.getByLocalId('manual:abc'))!.localDeleted, isTrue);
  });
