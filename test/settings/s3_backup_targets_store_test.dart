...
void main() {
  ...
  test('add twice keeps both targets, each with a distinct id', () async {
    ...
    await store.add(accessKeyId: 'a', secretAccessKey: 'b', region: 'us-east-1', bucket: 'bucket-one', prefix: '');
    await store.add(accessKeyId: 'a', secretAccessKey: 'b', region: 'eu-west-1', bucket: 'bucket-two', prefix: '');
  ...
  test('remove deletes only the matching target', () async {
    final store = S3BackupTargetsStore(store: FakeSecureStore());
    final keep = await store.add(accessKeyId: 'a', secretAccessKey: 'b', region: 'r', bucket: 'keep-me', prefix: '');
    final drop = await store.add(accessKeyId: 'a', secretAccessKey: 'b', region: 'r', bucket: 'drop-me', prefix: '');
