...
- [ ] T4.1 Photo grid (camera roll) with backup-status badges, thumbnail-first progressive loading — see `lib/viewer/library_screen.dart` — depends: T2.2, T3.2. **Visual shell done**: real Photos-app-style single-page IA (one scrollable page, no bottom tab bar — day-grouped square grid up top, then Media Types + Utilities grouped lists), swipe viewer, long-press context menu for Favorite/Hide/Delete. Favorites/Hidden/Recently Deleted are real (`lib/viewer/favorites_screen.dart`, `hidden_screen.dart`, `recently_deleted_screen.dart`, soft-delete + restore on `asset_record`) — but the grid still lists manually-added files, not a real camera-roll grid (needs `photo_manager`, T2.1), and video tiles are a placeholder icon, not an extracted frame (needs T2.2/T2.3). No Albums/People & Pets — no data to back those.
